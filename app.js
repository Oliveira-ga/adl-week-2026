...
function memoryPage(){
  ...
  if(m.status==='result') return resultPage('Memória','Resultado do jogo',m.awards,m.confirmed,'confirm-memory');
...
function initQuiz(){ const selected=shuffle(QUESTIONS).slice(0,3); state.quiz={status:'bet',index:0,questions:selected,bets:{},answers:{},results:[],locked:false,confirmed:false,awards:null}; save(); }
function quizPage(){const qz=state.quiz;if(!qz)return introGame('Dinâmica 2','Quiz de Apostas','Três perguntas. Acertou: ganha 2× a aposta. Errou: perde a aposta.','start-quiz');if(qz.status==='result')return resultPage('Quiz','Resultado do quiz',qz.awards,qz.confirmed,'confirm-quiz',qz.results);const q=qz.questions[qz.index];if(qz.status==='bet')return `<main class="page"><p class="eyebrow">Pergunta ${qz.index+1} de 3 · Fase de aposta</p><h1 class="question">${esc(q.q)}</h1><p class="notice">Registre apostas em múltiplos de 10. O máximo é a pontuação atual do time.</p><section class="grid bet-grid spaced">${state.teams.map(t=>`<div class="panel bet-card" style="--team:${t.color}"><h2>${esc(t.name)}</h2>${t.score===0?`<div class="hidden-bet">Sem pontos</div>`:`<label>Aposta<input data-bet="${t.id}" type="number" min="10" max="${t.score}" step="10" value="${qz.bets[t.id]||10}"></label><div class="${state.showBets?'bet-value':'hidden-bet'}">${state.showBets?`${qz.bets[t.id]||10} pts`:'Aposta oculta'}</div>`}</div>`).join('')}</section><button id="lock-bets" class="btn primary spaced">Travar apostas e mostrar opções</button></main>`;return `<main class="page"><p class="eyebrow">Pergunta ${qz.index+1} de 3 · Resposta</p><h1 class="question">${esc(q.q)}</h1><section class="options">${q.o.map((o,i)=>`<button class="option ${qz.revealed?(i===q.a?'correct':'wrong'):''}" data-option="${i}" ${qz.revealed?'disabled':''}><b>${'ABCD'[i]}.</b> ${esc(o)}</button>`).join('')}</section>${qz.revealed?`<section class="panel spaced"><h2 class="success-text">Resposta: ${'ABCD'[q.a]}</h2><p>${esc(q.e)}</p><div class="grid bet-grid">${state.teams.map(t=>`<label style="color:${t.color}">${esc(t.name)} respondeu<select data-answer="${t.id}"><option value="correct">Corretamente</option><option value="wrong">Errado</option></select></label>`).join('')}</div><button id="apply-answer" class="btn primary spaced">Aplicar resultado</button></section>`:'<p class="muted spaced">Clique na alternativa correta para revelar.</p>'}</main>`;}
...
function resultPage(game,title,awards,confirmed,id,rows=[]){return `<main class="page"><p class="eyebrow">${game} · Resultado</p><h1>${title}</h1><section class="grid score-grid">${state.teams.map(t=>`<div class="panel score-card" style="--team:${t.color}"><h2>${esc(t.name)}</h2><div class="score">${signed(awards[t.id]||0)}<small> pts</small></div></div>`).join('')}</section>${rows.length?`<section class="panel spaced"><h2>Detalhamento</h2><table><thead><tr><th>Etapa</th>${state.teams.map(t=>`<th>${esc(t.name)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.label)}</td>${state.teams.map(t=>`<td>${signed(r.deltas[t.id]||0)}</td>`).join('')}</tr>`).join('')}</tbody></table></section>`:''}<section class="panel center"><p>${confirmed?'Resultado já adicionado ao placar.':'Revise o resultado antes de confirmar.'}</p><button id="${id}" class="btn primary" ${confirmed?'disabled':''}>${confirmed?'Adicionado ao placar':'Confirmar e adicionar ao placar'}</button><button class="btn ghost" data-nav="scoreboard">Voltar ao placar</button></section></main>`;}
